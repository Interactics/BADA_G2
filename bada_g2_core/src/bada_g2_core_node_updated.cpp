	...
	while (ros::ok())
	{
		...
		case FINDING_PEPL:
			...
			do
			{
				...
				{	
					ROS_INFO("break");
					if(finding_pepl_fail)
					{
						state = MOVING_TO_PEPL;
						move_to_aligned = true;
					}
					break;
				}
				current++;
		...
		case SOUND_DETECTING:
			...
			// break;
		case MOVING_TO_PEPL:
			//MOVING_TO_PEPL에서 사람을 못 찾은 게 아니라 정상작동인 경우
			if(!move_to_aligned)
			{
				bada_display_cmd(DISP_EVNT::A_UP);
				waitSec(1);
				...
				//사람 체크
				is_there_pepl = bada_rounding();
				//사람이 없으면 FINDING_PEPL로 돌아가기
				if (!is_there_pepl)
				{
					ROS_INFO("no ppl here");
					state = FINDING_PEPL;
			...
			//MOVING_TO_PEPL에서 사람을 못 찾았었던 경우
			//	= FINDING_PEPL에서 다시 돌아온 경우
			else
			{
				bada_aligned_pepl(); // 사람의 위치고 로봇 사람을 가운데로
				ROS_INFO("align ppl done");
