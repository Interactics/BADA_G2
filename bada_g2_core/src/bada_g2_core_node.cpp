...
int main(int argc, char **argv)
{
	...
	// bada_roaming();
	// return 0;
	bada_display_cmd(DISP_EVENT::LOGO);
	waitSec(3);
	...
	while (ros::ok())
	{
		...
		case FINDING_PEPL: //turn & findpeople
			ROS_INFO("starting FINDING_PEOPLE_DEMO");
			do
			{
		...
		case ROAMING:
			...
			// bada_display_cmd(DISP_EVENT::A_UP);
			// waitSec(1);
			// bada_display_cmd(DISP_EVENT::NOTHING);
			bada_roaming_demo(); // 카테고리에 등록된 소리가 나올 때까지 배회하기
			waitSec(1);
			// waitSec(1);
			...
			// 	bada_display_cmd(DISP_EVENT::CRYING_EVENT);
			// } else if(LastSignal.data == "Alarm") {
			// 	bada_display_cmd(DISP_EVENT::FIRE_EVENT);
			// }
			waitSec(1);
			bada_display_cmd(DISP_EVENT::NOTHING);
			waitSec(0.2);
		...
		case MOVING_TO_PEPL:
			...
			waitSec(1);
			bada_go_to_pepl_demo(); // 반경 2m 이내 도달 검사하기. 그렇지 않으면 계속 접근
		...
		case MOVING_WITH_PEPL:
			...
			else if (LastSignal.data == "Door")
			{
				bada_display_cmd(DISP_EVENT::DOOR_EVENT);
			}
			else if (LastSignal.data == "Water")
			{
				bada_display_cmd(DISP_EVENT::WATER_EVENT);
			}
			else if (LastSignal.data == "Bell")
			{
				bada_display_cmd(DISP_EVENT::BELL_EVENT);
			}
			else
			{
				ROS_INFO("ERROR");
			...
			bada_go_to_soundPT_demo(); // 소리가 발생한 지점으로 이동하기.
			ROS_INFO("go to sound pt done");

			if (LastSignal.data == "Cry")
			{
				bada_display_inform(DISP_EVENT::CRYING_EVENT);
			}
			else if (LastSignal.data == "Alarm")
			{
				bada_display_inform(DISP_EVENT::FIRE_EVENT);
			}
			else if (LastSignal.data == "Door")
			{
				bada_display_inform(DISP_EVENT::DOOR_EVENT);
			}
			else if (LastSignal.data == "Water")
			{
				bada_display_inform(DISP_EVENT::WATER_EVENT);
			}
			else if (LastSignal.data == "Bell")
			{
				bada_display_inform(DISP_EVENT::BELL_EVENT);
			}
			else
			{
				ROS_INFO("ERROR");
				bada_display_inform(DISP_EVENT::LOGO);
			}
